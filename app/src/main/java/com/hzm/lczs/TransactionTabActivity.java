...
public class TransactionTabActivity extends Activity implements OnClickListener, OnCheckedChangeListener, OnItemSelectedListener {
    ...
    private RadioGroup trans_type_tab_rg = null;//收入？支出
    ...
    private void loadingFormation() {
        ...
        trans_type_tab_rg = (RadioGroup) findViewById(R.id.trans_type_tab_rg);//收入？支出
        rb1 = (RadioButton) findViewById(R.id.payout_tab_rb);
        rb1.setChecked(true);
        rb1.setOnCheckedChangeListener(this);

        cost_btn = (Button) findViewById(R.id.cost_btn);//输入金额
        cost_btn.setOnClickListener(this);

        corporation_fl = (FrameLayout) findViewById(R.id.corporation_fl);//商家布局
        empty_fl = (FrameLayout) findViewById(R.id.empty_fl);//按钮布局


        first_level_category_spn = (Spinner) findViewById(R.id.first_level_category_spn);//类别
        first_level_category_spn.setOnItemSelectedListener(this);

        sub_category_spn = (Spinner) findViewById(R.id.sub_category_spn);//子类别
        sub_category_spn.setOnItemSelectedListener(this);

        account_spn = (Spinner) findViewById(R.id.account_spn);//账户

        corporation_spn = (Spinner) findViewById(R.id.corporation_spn);//商家

        trade_time_btn = (Button) findViewById(R.id.trade_time_btn);//交易时间
        trade_time_btn.setOnClickListener(this);

        project_spn = (Spinner) findViewById(R.id.project_spn);//项目

        memo_btn = (Button) findViewById(R.id.memo_btn);//备注布局
    ...
    private void updateInfo(int position) {
        ...
        if (position < 0) {
            ...
            db.open();
            cursor = db.select(MyDbInfo.getTableNames()[6], MyDbInfo.getFieldNames()[6],
                    "(select POSTIVE from TBL_ACCOUNT_TYPE where ID=" +
                            MyDbInfo.getFieldNames()[6][2] + ")" +
        ...
        db.open();
        cursor = db.select(MyDbInfo.getTableNames()[id_sub], MyDbInfo.getFieldNames()[id_sub],
                MyDbInfo.getFieldNames()[id_sub][2] + "=?",
...
//				account_spn.setSelection(data.account_id-1);
                for (int i = 0; i < accountId.length; i++)
                    if (String.valueOf(data.account_id).equals(accountId[i]))
                        account_spn.setSelection(i);
